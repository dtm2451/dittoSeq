...
dittoBarPlot <- function(
    ...
    # Rename and/or reorder y.var groupings
    var.args <- list(x = data$label)
    if (!(is.null(var.labels.reorder))) {
        var.args$levels <- levels(factor(var.args$x))[var.labels.reorder]
    }
    if (!(is.null(var.labels.rename))) {
        var.args$labels <- var.labels.rename
    }
    data$label <- do.call(factor, args = var.args)
