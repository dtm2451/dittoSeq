...
dittoDimPlot <- function(
    ...
    add.trajectory.lineages = NULL, add.trajectory.curves = NULL, trajectories.cluster.meta,
    ...
    if (is.list(add.trajectory.lineages)) {
        p <- .add_trajectory_lineages(
            p, add.trajectory.lineages, trajectories.cluster.meta,
            trajectories.arrow.size, object, reduction.use, dim.1, dim.2)}

    if (is.list(add.trajectory.curves)) {
        p <- .add_trajectory_curves(
            p, add.trajectory.curves, trajectories.arrow.size, reduction.use, dim.1, dim.2)}
...
.add_trajectory_curves <- function(
    ...
    x <- paste0(reduction.use, "_", dim.1)
    y <- paste0(reduction.use, "_", dim.2)
