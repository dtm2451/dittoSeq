...
DittoDimPlot <- function(var="ident", object = DEFAULT, reduction.use = NA, dim.1 = 1, dim.2 = 2,
  ...
  p.df <- DittoScatterPlot(x.var = xdat$embeddings, y.var = ydat$embeddings, overlay.color.var = var,
                           ...
                           out.List = TRUE)
  p <- p.df$plot
  Target_dat <- p.df$Target_dat

  #Add letters if warranted
  if(do.letter){
    letter.labels <- c(LETTERS, letters, 0:9, "!", "@", "#", "$", "%", "^", "&", "*", "(", ")", "-",
                       "+", "_", "=", ";", "/", "|", "{", "}", "~")[seq_along(levels(as.factor(Target_dat$color)))]
    names(letter.labels) <- levels(as.factor(Target_dat$color))
    letter.colors <- c(rep("white",7), rep("black",10),rep("white",9))[seq_along(levels(as.factor(Target_dat$color)))]
    names(letter.colors) <- levels(as.factor(Target_dat$color))
    p <- p +
      geom_point(data=Target_dat, aes(x = X, y = Y, shape = color), color = "black", size=size, alpha = opacity*3/4) +
...
DittoDimPlot.addLabels <- function(p, Target_dat, highlight.labels, rename.groups, label.size) {
  ...
  cent.2 = sapply(levels(as.factor(Target_dat[,3])), function(X) median(Target_dat$Y[Target_dat[,3]==X]))
  #Add labels
  if (highlight.labels){
    #Add labels with a white background
    p <- p +
      geom_label(data = data.frame(x=cent.1, y=cent.2),
                 aes(x = x, y = y, label = if(!(is.na(rename.groups[1]))){rename.groups} else {levels(as.factor(Target_dat[,3]))}),
                 size = label.size)
  } else {
